    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                # Calculate PSNR and SSIM manually
                from evaluation.psnr_ssim import calculate_psnr, calculate_ssim
                
                # Convert tensors to numpy and normalize to [0, 1]
                gt_np = ((self.gt_image.detach().cpu().numpy() + 1) / 2.0).clip(0, 1)
                out_np = ((self.output.detach().cpu().numpy() + 1) / 2.0).clip(0, 1)
                
                # Calculate PSNR and SSIM for each image in batch
                for i in range(gt_np.shape[0]):
                    psnr_val = calculate_psnr(gt_np[i], out_np[i], crop_border=0, test_y_channel=False)
                    ssim_val = calculate_ssim(gt_np[i], out_np[i], crop_border=0, test_y_channel=False)
        ...
        # Print results BEFORE logger cleanup
        print("\n" + "="*60)
        ...
        test_log.update({'psnr': avg_psnr, 'ssim': avg_ssim, 'mae': avg_mae})
